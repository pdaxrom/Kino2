...
static void ipp_idct_ARM_put(uint8_t *dest, int line_size, DCTELEM *block)
{
    ippiDCTInv_8x8_16s_I(block);
    ff_put_pixels_clamped(block, dest, line_size);
}
static void ipp_idct_ARM_add(uint8_t *dest, int line_size, DCTELEM *block)
{
    ippiDCTInv_8x8_16s_I(block);
...
#ifdef USE_IPP
    fprintf(stderr, "ffmpeg:  using Intel PXA IPP IDCT transform\n");
    c->idct_put= ipp_idct_ARM_put;
    c->idct_add= ipp_idct_ARM_add;
    c->idct    = (void *) ippiDCTInv_8x8_16s_I;
    c->idct_permutation_type= FF_NO_IDCT_PERM;
#else
    if(idct_algo==FF_IDCT_AUTO || idct_algo==FF_IDCT_ARM){
	fprintf(stderr, "ffmpeg:  using j_rev_dct transform\n");
        c->idct_put= j_rev_dct_ARM_put;
        c->idct_add= j_rev_dct_ARM_add;
	c->idct    = j_rev_dct_ARM;
        c->idct_permutation_type= FF_LIBMPEG2_IDCT_PERM;/* FF_NO_IDCT_PERM */
    } else if (idct_algo==FF_IDCT_SIMPLEARM){
	fprintf(stderr, "ffmpeg:  using simple_idct transform\n");
